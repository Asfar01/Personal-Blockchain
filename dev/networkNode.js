...
app.post("/transaction", function (req, res) {
  ...
  res.json({
    note: `The transaction will be added in block number ${blockIndex}.`,
  });
});

//broadcasts the transaction to the whole network
app.post("/transaction/broadcast", function (req, res) {
  const newTransaction = sikay.createNewTransaction(
    req.body.amount,
    req.body.sender,
    req.body.recipient
  );
...
app.get("/mine", function (req, res) {
  ...
  const blockHash = sikay.calculateHash(
    previousBlockHash,
    nonce,
    currentBlockData
  );
...
app.post("/register-and-broadcast-node", function (req, res) {
  ...
  if (sikay.networkNodes.indexOf(newNodeUrl) == -1)
    sikay.networkNodes.push(newNodeUrl);
  ...
  Promise.all(regNodePromises)
    .then((data) => {
      const bulkRegisterOptions = {
        ...
        method: "POST",
        body: {
          allNetworkNodes: [...sikay.networkNodes, sikay.currentNodeUrl],
        },
        json: true,
      };
...
app.post("/register-node", function (req, res) {
  ...
  if (sikay.networkNodes.indexOf(newNodeUrl) == -1 && notCurrentNode)
    sikay.networkNodes.push(newNodeUrl);
...
app.post("/register-node-bulk", function (req, res) {
  ...
  allNetworkNodes.forEach((networkNodeUrl) => {
    ...
    if (nodeNotPresent && notCurrentNode)
      sikay.networkNodes.push(networkNodeUrl);
  });
...
app.get("/consensus", function (req, res) {
  ...
  Promise.all(requestPromises).then((blockchains) => {
    ...
    if (
      !newLongestChain ||
      (newLongestChain && !sikay.chainIsValid(newLongestChain))
    ) {
      res.json({
